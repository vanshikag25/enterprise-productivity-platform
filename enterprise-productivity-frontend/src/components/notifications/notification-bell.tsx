...
export function NotificationBell() {
  ...
  const measurePosition = useCallback(() => {
    ...
    setPosition({
      ...
      right: Math.max(PANEL_MIN_EDGE, window.innerWidth - rect.right),
    });
  }, []);
    ...
    isOpen && position && typeof document !== 'undefined'
      ? createPortal(
          <div
            ...
            className="fixed z-[80] max-h-[28rem] w-[min(22rem,calc(100vw-1.5rem))] overflow-hidden rounded-xl border border-slate-100 bg-white shadow-popover animate-scale-in"
          >
  ...
  return (
    <div className="relative">
      <button
        ...
        className={`btn-icon btn-ghost relative rounded-lg transition-colors ${isOpen ? 'bg-slate-100 text-slate-700' : 'text-slate-400 hover:text-slate-600'}`}
      >
