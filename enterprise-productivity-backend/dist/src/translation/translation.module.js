...
function buildProvider(configService) {
    ...
    if (provider === 'gemini' || provider === 'openai') {
        ...
        if (apiKey) {
            return new openai_translation_provider_1.OpenAiTranslationProvider(apiKey, configService.get('ai.geminiBaseUrl') ??
        ...
        new common_1.Logger('TranslationModule').warn('AI_PROVIDER=gemini/openai is set but the API key is missing; falling back to the mock provider.');
    }
