...
function buildProvider(configService) {
    ...
    if (provider === 'openai') {
        ...
        if (apiKey) {
            return new openai_sentiment_provider_1.OpenAiSentimentProvider(apiKey, configService.get('ai.openaiBaseUrl') ??
                'https://api.openai.com/v1', configService.get('ai.openaiModel') ?? 'gpt-4o-mini');
        }
        new common_1.Logger('SentimentModule').warn('AI_PROVIDER=openai is set but OPENAI_API_KEY is missing; falling back to the mock provider.');
    }
