...
/**
 * Offline keyword dictionary used by the mock provider. Each entry is
 * `english phrase` -> native phrase for that language. Keep entries short so
 * they read naturally inside longer messages.
 */
type PhraseMap = Record<string, string>;

const PHRASES: Record<string, PhraseMap> = {
  hi: {
    hello: 'namaste',
    thank: 'dhanyavad',
    thanks: 'dhanyavad',
...
const SCRIPT_LANGUAGES: { code: string; test: RegExp }[] = [
  { code: 'hi', test: /[\u0900-\u097F]/ },
  { code: 'ar', test: /[\u0600-\u06FF]/ },
  { code: 'ru', test: /[\u0400-\u04FF]/ },
  { code: 'ja', test: /[\u3040-\u30FF]|[\u4E00-\u9FFF]/ },
...
/**
 * Detects the source language from Unicode ranges with which scripts the
 * message actually uses, then applies a small English->native phrase dictionary.
 * Unmatched words pass through unchanged; when nothing can be translated the
 * original text is returned with a clear offline notice so callers can handle
 * it gracefully.
 */
@Injectable()
export class MockTranslationProvider implements TranslationProvider {
  ...
  async translate(request: TranslationRequest): Promise<TranslationResult> {
    ...
    if (!phrases) {
      throw new Error(
        `Translation into "${request.targetLanguage}" is not supported by the offline provider.`,
      );
    }
  ...
  private detectSourceLanguage(text: string): string | null {
    for (const lang of SCRIPT_LANGUAGES) {
      if (lang.test.test(text)) return lang.code;
    }
    const latinWords = (text.match(/[A-Za-z]+/g) ?? []).length;
    return latinWords >= 2 ? 'en' : null;
  }

  private applyDictionary(text: string, phrases: PhraseMap): string {
    let out = text;
