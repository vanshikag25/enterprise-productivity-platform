...
__export(index_exports, {
  ...
  MENTION_ONLY_COMMANDS: () => MENTION_ONLY_COMMANDS,
  ...
  defaultCommandSendabilityValidator: () => defaultCommandSendabilityValidator,
  ...
  getMentionedUsersInText: () => getMentionedUsersInText,
  ...
  notifyCommandNotReady: () => notifyCommandNotReady,
  ...
  stripMentionTokens: () => stripMentionTokens,
...
// src/messageComposer/middleware/textComposer/mentionUtils.ts
var isUserMentionEntity = (entity) => entity.mentionType === "user";
var userResponseToMentionEntity = (user) => ({
  ...user,
  mentionType: "user"
});
var userResponsesToMentionEntities = (users) => users.map(userResponseToMentionEntity);
var mentionEntityToUserResponse = (entity) => {
...
var userSuggestionToMentionEntity = (suggestion) => userResponseToMentionEntity(userSuggestionToUserResponse(suggestion));
...
var getInitialMentions = (message) => {
  if (!message) return [];
  const mentions = (message.mentioned_users ?? []).map(
    (item) => typeof item === "string" ? { id: item, mentionType: "user" } : { ...item, mentionType: "user" }
  );
...
var isSameMentionEntity = (first, second) => first.id === second.id && first.mentionType === second.mentionType;
var getMentionsFromState = (state) => state.mentions ?? state.mentionedUsers.map(userResponseToMentionEntity);
var initState2 = ({
  composer,
  message
}) => {
...
var TextComposer = class {
  constructor({ composer, message }) {
     ...
     */
    this.getMentionedUser = (userId) => this.mentionedUsers.find((user) => user.id === userId);
    /**
    ...
    this.upsertMentionEntity = (entity) => {
      const mentions = [...this.mentions];
      const existingEntityIndex = mentions.findIndex(
        (currentEntity) => isSameMentionEntity(currentEntity, entity)
      );
    ...
    this.getMentionEntity = (mentionType, id) => this.mentions.find(
      (entity) => entity.mentionType === mentionType && entity.id === id
    );
    this.removeMentionEntity = (mentionType, id) => {
  ...
  setMentions(entities) {
    this.state.partialNext({
      mentionedUsers: entities.filter(isUserMentionEntity).map(mentionEntityToUserResponse),
      mentions: entities
    });
  }
  clearCommand() {
...
var UserGroupPaginator = class extends BasePaginator {
  constructor(client, options) {
    super(options);
    this.buildNextCursor = (items) => {
      if (items.length < this.pageSize) return void 0;
    ...
    this.query = async ({
      ...
      const cursor = decodeCursor(this.cursor?.next);
      const options = {
        limit: this.pageSize,
...
var stripMentionTokens = (text, mentionedUsersInText, trigger = "@") => mentionedUsersInText.reduce((value, user) => {
...
var getMentionedUsersInText = (text, mentionedUsers) => Array.from(
  new Set(
    mentionedUsers.filter(
      ({ id, name }) => text.includes(`@${id}`) || !!name && text.includes(`@${name}`)
    )
  )
);
...
var createCompositionValidationMiddleware = (composer, commandSearchSource) => {
  const effectiveCommandSearchSource = commandSearchSource ?? new CommandSearchSource(composer.channel);
  return {
    ...
    handlers: {
      compose: async ({
        ...
        const currentCommand = composer.textComposer.command ?? getCommandByName(effectiveCommandSearchSource, getRawCommandName(inputText));
...
var textIncludesMentionToken = (text, token) => text.includes(`@${token}`);
var isDefined = (value) => value !== void 0;
var getMentionEntityTextCandidates = (entity) => {
...
var isMentionEntityPresentInText = (entity, text) => {
...
var buildMentionCompositionMetadata = ({
  mentions,
  text
}) => {
...
var createTextComposerCompositionMiddleware = (composer) => ({
  ...
  handlers: {
    compose: ({
      ...
      if (!text && !mentioned_channel && !mentioned_here && mentioned_group_ids.length === 0 && mentioned_roles.length === 0 && mentioned_users.length === 0) {
        return forward();
      }
      return next({
        ...state,
  ...
  handlers: {
    compose: ({
      ...
      const draftMentionPayload = {
        ...
        ...mentioned_users.length ? { mentioned_users: mentioned_users.map((u) => u.id) } : {}
      };
      return next({
        ...state,
        draft: {
...
var upsertUserResponse = (users, user) => {
...
var upsertMentionEntity = (mentions, entity) => {
  const existingIndex = mentions.findIndex(
    (currentEntity) => currentEntity.id === entity.id && currentEntity.mentionType === entity.mentionType
  );
...
var mentionSuggestionToEntity = (suggestion) => {
...
var mentionSuggestionToInsertText = (suggestion) => `@${suggestion.name || suggestion.id} `;
var DEFAULT_SUGGESTION_FACTORY_MAPPERS = {
...
var MentionsSearchSource = class extends BaseSearchSource {
  constructor(channel, options) {
    ...
    super(restOptions);
    this.type = "mentions";
    this.normalizeSearchValue = (value) => this.transliterate(removeDiacritics(value)).toLowerCase();
    this.matchesSearchQuery = (value, searchQuery) => {
      if (!searchQuery) return true;
      return this.normalizeSearchValue(value).includes(
        this.normalizeSearchValue(searchQuery)
      );
    };
    this.matchesPrefixSearchQuery = (value, searchQuery) => {
      if (!searchQuery) return true;
      return this.normalizeSearchValue(value).startsWith(
        this.normalizeSearchValue(searchQuery)
      );
    };
    this.matchesUserNameSearchQuery = (value, searchQuery) => {
      if (!searchQuery) return true;
      const normalizedValueWords = this.normalizeSearchValue(value).split(/\s+/).filter(Boolean);
      const normalizedQueryWords = this.normalizeSearchValue(searchQuery).split(/\s+/).filter(Boolean);
    ...
    this.getChannelTeam = () => this.channel.data?.team;
    this.toUserSuggestion = (user, searchToken = this.searchQuery) => this.mapMentionSuggestion("user", user, searchToken);
    this.toChannelMentionSuggestion = (searchToken = this.searchQuery) => this.mapMentionSuggestion("channel", "channel", searchToken);
    this.toHereMentionSuggestion = (searchToken = this.searchQuery) => this.mapMentionSuggestion("here", "here", searchToken);
    this.toRoleMentionSuggestion = (role, searchToken = this.searchQuery) => this.mapMentionSuggestion("role", role, searchToken);
    this.toUserGroupMentionSuggestion = (userGroup, searchToken = this.searchQuery) => this.mapMentionSuggestion("user_group", userGroup, searchToken);
    this.canExecuteQuery = (newSearchString) => {
      const hasNewSearchQuery = typeof newSearchString !== "undefined";
    ...
    this.getBuiltinMentionSuggestions = (searchQuery) => [
      ...this.isMentionTypeAllowed("channel") ? [this.toChannelMentionSuggestion(searchQuery)] : [],
      ...this.isMentionTypeAllowed("here") ? [this.toHereMentionSuggestion(searchQuery)] : []
    ].filter(({ name }) => this.matchesPrefixSearchQuery(name, searchQuery));
    this.getRoleMentionSuggestions = async (query) => {
      if (!this.isMentionTypeAllowed("role")) return [];
      if (!query) return [];
      const { roles } = await this.client.searchRoles({ query });
      return [...roles?.map((role) => role.name) ?? []].sort((left, right) => left.localeCompare(right)).map((role) => this.toRoleMentionSuggestion(role, query));
    };
    ...
    this.prepareQueryUsersParams = (searchQuery, offset = 0) => ({
      filters: {
        $or: [
          { id: { $autocomplete: searchQuery } },
          { name: { $autocomplete: searchQuery } }
    ...
    this.prepareQueryMembersParams = (searchQuery, offset = 0) => {
    ...
    this.queryUsers = async (searchQuery, offset = 0) => {
      const { filters, sort, options } = this.prepareQueryUsersParams(searchQuery, offset);
      const { users } = await this.client.queryUsers(filters, sort, options);
      return users;
    };
    this.queryMembers = async (searchQuery, offset = 0) => {
    ...
    this.getUserSuggestionsPage = async (searchQuery, userOffset = 0) => {
      ...
      if (this.config.mentionAllAppUsers) {
        ...
        const items2 = localUsers.slice(userOffset, userOffset + this.pageSize).map((user) => this.toUserSuggestion(user, searchQuery));
        return {
          items: items2,
          nextOffset: localUsers.length > userOffset + this.pageSize ? userOffset + items2.length : void 0
        };
      } else {
      ...
      return {
        items,
        nextOffset: users.length === this.pageSize ? userOffset + users.length : void 0
      };
    };
    this.buildUserGroupSearchCursor = (items) => {
      if (items.length < this.pageSize) return void 0;
      const lastItem = items[items.length - 1];
      if (!lastItem?.name) return void 0;
      return JSON.stringify({
        id_gt: lastItem.id,
        name_gt: lastItem.name
      });
    };
    this.getUserGroupSuggestionsPage = async (searchQuery, cursor) => {
      ...
      const userGroupCursor = decodeUserGroupCursor(cursor);
      const options = {
        query: searchQuery,
        limit: this.pageSize,
      ...
      return {
        items: user_groups.map(
          (userGroup) => this.toUserGroupMentionSuggestion(userGroup, searchQuery)
        ),
  ...
  updatePaginationStateFromQuery() {
    const userPaginationState = this.latestUserPaginationState ?? { itemCount: 0 };
    return {
      hasNext: typeof userPaginationState.nextOffset !== "undefined" || typeof this.userGroupCursor !== "undefined",
      next: void 0,
  ...
  async query(searchQuery) {
    ...
    const [userResultsState, userGroupResultsState, roleSuggestionsState] = await Promise.allSettled([
      this.getUserSuggestionsPage(searchQuery, userOffset),
      this.getUserGroupSuggestionsPage(searchQuery, previousUserGroupCursor),
      isFirstPage ? this.getRoleMentionSuggestions(searchQuery) : Promise.resolve([])
    ]);
    const userResults = userResultsState.status === "fulfilled" ? userResultsState.value : {
      items: [],
      nextOffset: isFirstPage ? void 0 : previousUserPaginationState?.nextOffset
    };
    const userGroupResults = userGroupResultsState.status === "fulfilled" ? userGroupResultsState.value : {
      items: [],
      next: isFirstPage ? void 0 : previousUserGroupCursor
    };
    const roleSuggestions = roleSuggestionsState.status === "fulfilled" ? roleSuggestionsState.value : [];
    const items = [
  ...
  filterMutes(data) {
    ...
    if (textComposerText.includes("/unmute")) {
      return data.filter(
        (suggestion) => suggestion.mentionType === "user" && mutedUsers.some((mute) => mute.target.id === suggestion.id)
      );
    }
    return data.filter(
      (suggestion) => suggestion.mentionType !== "user" || mutedUsers.every((mute) => mute.target.id !== suggestion.id)
    );
  }
  filterQueryResults(items) {
  ...
  resetState() {
    this.latestUserPaginationState = void 0;
    this.userGroupCursor = void 0;
    super.resetState();
  }
};
...
var createMentionsMiddleware = (channel, options) => {
  ...
  searchSource.activate();
  let lastInsertedMentionEndOffset;
  return {
    id: "stream-io/text-composer/mentions-middleware",
    handlers: {
      onChange: ({ state, next, complete, forward }) => {
        if (!state.selection) return forward();
        const cursorJustInsertedAMention = lastInsertedMentionEndOffset !== void 0 && state.selection.end === lastInsertedMentionEndOffset;
        lastInsertedMentionEndOffset = void 0;
        const currentMentions = state.command || state.text.trimStart().startsWith("/") ? state.mentionedUsers : getMentionedUsersInText(state.text, state.mentionedUsers);
        const mentionedUsersChanged = currentMentions.length !== state.mentionedUsers.length || currentMentions.some(
          (user, index) => user.id !== state.mentionedUsers[index]?.id
        );
        const stateWithMentions = mentionedUsersChanged ? { ...state, mentionedUsers: currentMentions } : state;
        ...
        const triggerMatchesCommittedMention = !!triggerWithToken && /\s$/.test(textBeforeCursor) && (cursorJustInsertedAMention || (stateWithMentions.mentions ?? []).some((entity) => {
        ...
        if (triggerWasRemoved || triggerMatchesCommittedMention) {
          const hasStaleSuggestions = stateWithMentions.suggestions?.trigger === finalOptions.trigger;
      ...
      onSuggestionItemSelect: ({ state, complete, forward }) => {
        ...
        return complete({
          ...state,
          ...insertResult,
          mentionedUsers: selectedSuggestion.mentionType === "user" ? upsertUserResponse(
...
// src/messageComposer/configuration/commands.configuration.ts
var MENTION_ONLY_COMMANDS = /* @__PURE__ */ new Set(["mute", "unmute", "unban"]);
var defaultCommandSendabilityValidator = ({
  command,
  commandArgsText,
...
var applyCommandValidatorOverride = (targetConfig, sourceConfig) => {
  const overrideValidator = sourceConfig?.commands?.sendValidator;
...
var _LinkPreviewsManager = class _LinkPreviewsManager {
  constructor({ composer, message }) {
    ...
    this.initState = ({ message } = {}) => {
      ...
      this.state.next(initState4({ message: this.enabled ? message : void 0 }));
    };
    this.getSnapshot = () => this.state.getLatestValue();
    this.restoreSnapshot = (snapshot) => {
      this.state.next(snapshot);
    ...
    this.composer = composer;
    this.state = new StateStore(
      initState4({ message: this.enabled ? message : void 0 })
    );
...
var LocationComposer = class {
  constructor({ composer, message }) {
    this.initState = ({ message } = {}) => {
      this.state.next(initState5({ message }));
    };
    this.getSnapshot = () => this.state.getLatestValue();
    this.restoreSnapshot = (snapshot) => {
      this.state.next(snapshot);
    ...
    this.composer = composer;
    this.state = new StateStore(initState5({ message }));
...
var _MessageComposer = class _MessageComposer extends WithSubscriptions {
  ...
  constructor({
    ...
    this.validateCommandSendability = (command, text = this.textComposer.text) => {
      ...
      const validationContext = {
        command,
        commandArgsText: command.name ? stripCommandFromText(text, command.name).trim() : text.trim(),
        composer: this,
        mentionedUsersInText,
        rawText: text
      };
    ...
    const mergeMessageComposerConfigCustomizer = (originalVal, channelConfigVal, key) => typeof originalVal === "object" ? void 0 : originalVal === false && key === "enabled" ? false : ["string", "number", "bigint", "boolean", "symbol"].includes(
  ...
  get hasSendableData() {
    return this.isCommandSendable && !!(!this.attachmentManager.uploadsInProgressCount && (!this.textComposer.textIsEmpty || this.attachmentManager.successfulUploadsCount > 0) || this.pollId || !!this.locationComposer.validLocation);
  }
...
var StreamChat = class _StreamChat {
   ...
   */
  async queryUserGroups(options = {}) {
    return await this.get(this.baseURL + "/usergroups", options);
  }
  /**
   ...
   */
  async searchUserGroups(options) {
    return await this.get(
   ...
   */
  async updateUserGroup(id, options) {
   ...
   */
  async deleteUserGroup(id, options = {}) {
   ...
   */
  async addUserGroupMembers(id, options) {
    return await this.post(
   ...
   */
  async removeUserGroupMembers(id, options) {
    return await this.post(
   ...
   */
  searchRoles(options) {
    return this.get(`${this.baseURL}/roles/search`, options);
  }
...
0 && (module.exports = {
  ...
  decodeSnsPayload,
  decodeSqsPayload,
  defaultCommandSendabilityValidator,
  defaultPollFieldBlurEventValidators,
  defaultPollFieldChangeEventValidators,
  encodeBase64,
